			...
			<div className={`${prefixCls}-content ${current === 1 ? 'has-footer' : ''}`}>
				...
				<div className={`${prefixCls}-form-items`}>
					<div className={`${prefixCls}-step-content ${current !== 0 ? 'hidden' : ''}`}>
						<Form.Item extra='与颁发商业许可证书或企业注册证书上需一致。' style={{ marginBottom: 24 }}>
							{getFieldDecorator('organizationName', {
								rules: [{ required: true, message: '请选择企业', transform: whitespaceTrim }]
							})(<Select
								placeholder='请输入关键字'
								...
								style={{ width: 448 }}
							>
								{list.map(it => <Select.Option key={it.id} dataRef={it} value={it.name}>{it.name}</Select.Option>)}
							</Select>)}
						</Form.Item>
						<Button onClick={this.onClickNextBtn} type='primary'>下一步</Button>
					</div>
					<div className={`${prefixCls}-step-content ${current !== 1 ? 'hidden' : ''}`}>
						<Spin spinning={formFieldsLoading}>
							{!formFieldsLoading && (
								formFields.length ? <div className={`${prefixCls}-custom-form`}>
									<FieldEdit
										form={this.props.form}
										formFields={formFields} changeFormFields={this.changeFormFields}
										validators={validators} changeValidators={this.changeValidators}
										communityId={this.communityId}
										status={status}
									/>
									...
									<Form.Item label='姓名'>
										{getFieldDecorator('name', {
											rules: [{ required: true, message: '请输入姓名', transform: whitespaceTrim }]
										})(<Input />)}
									</Form.Item>
									<Form.Item label='部门'>
									...
									<Form.Item label='邮箱'>
										{getFieldDecorator('email', {
											rules: [{ type: 'email', message: '请输入正确的邮箱', transform: whitespaceTrim }]
										})(<Input />)}
									</Form.Item>
								</div>
							)}
					...
					<div className={`${prefixCls}-success ${current !== 2 ? 'hidden' : ''}`}>
						...
						<Button type='primary' onClick={this.onClickBackBtn}>返回</Button>
					</div>
				</div>
				{current === 1 && <div className={`${prefixCls}-footer`}>
					<Button onClick={this.onClickPrevBtn}>上一步</Button>
					<Button type='primary' onClick={this.onClickSubmitBtn} loading={submiting}>确定</Button>
				</div>}
			</div>
		</Form>;
	}
